...
describe("buildRuntimeAdapters — SDK sessions", () => {
    ...
    test("forwards createAgentSession options from stage options", async () => {
        ...
        await adapters.agentSession!.create({
            ...
            NonNullable<typeof adapters.agentSession>["create"]
        >[0]);
        assert.equal(calls[0]?.cwd, "/repo");
        assert.deepEqual((calls[0] as unknown as { tools?: string[] })?.tools, [
            "read",
        ]);
        assert.equal(
    ...
    test("stage prompt output options do not override createAgentSession options", async () => {
        ...
        await stage.prompt("hello", {
            cwd: "/prompt-cwd",
            context: "fork",
            sessionDir: "/prompt-sessions",
        });

        assert.equal(calls[0]?.cwd, "/stage-cwd");
        assert.equal(
            (calls[0] as { context?: string } | undefined)?.context,
            undefined,
        );
        assert.equal(
            (calls[0] as { sessionDir?: string } | undefined)?.sessionDir,
            undefined,
        );
    });
    ...
    test("binds inherited theme and UI extension helpers onto stage sessions", async () => {
        ...
        const adapters = buildRuntimeAdapters(
            {
                ui: {
                    ...
                    getTheme: (name: string) =>
                        name === "host-theme" ? theme : undefined,
        ...
        assert.equal(
            (uiContext?.getTheme as (name: string) => unknown)("host-theme"),
            theme,
        );
        assert.equal(
            (uiContext?.setTheme as (name: string) => { success: boolean })(
                "host-theme",
            ).success,
            true,
        );
