...
`ralph` runs a goal-contract plan -> orchestrate -> simplify -> parallel review/audit loop. It infers the owner outcome, verification oracle, active work surface, and review criteria automatically, then keeps iterating until receipts and validation prove the oracle or the loop limit is reached.
...
| ------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ralph`                  | Goal-contract plan -> orchestrate -> simplify -> parallel review/audit loop that infers verifiable criteria, requires receipts, and keeps iterating until reviewers agree the oracle is satisfied or `max_loops` hits. Use it for implementation from a reviewed issue, goal, or spec. | `/workflow ralph prompt="Implement specs/2026-03-rate-limit.md and validate the behavior" max_loops=5`                                                                                        |
