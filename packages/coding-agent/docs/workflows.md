...
Atomic does not use the long-running `/goal` workflow by default for first-time workflow creation. If you explicitly choose `/goal` for reviewer-gated implementation, keep the objective tightly scoped with concrete done criteria and validation steps.
...
Do not spawn unbounded: dispatch in waves (for example 3–5 concurrent runs), and report the dispatch plan (item → run id → worktree) so the queue is auditable.
...
Use slash commands for graph connect and stage attach because those are interactive TUI surfaces.
