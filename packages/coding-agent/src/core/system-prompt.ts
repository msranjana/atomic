...
export function buildSystemPrompt(options: BuildSystemPromptOptions): string {
  ...
  - When passing large context from one workflow stage to another, write it to a file or artifact, pass the path with \`reads\`, and explicitly prompt the downstream agent with wording like \`Read the file at <path>...\`; do not inject large \`previous\` payloads or session history into the next prompt.
