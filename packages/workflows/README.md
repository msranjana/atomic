...
Graceful quit is idempotent for already-paused runs and preserves unresolved `ctx.ui` prompts. Stable author-callsite-and-composed-nested-scope reservations are created atomically before prompting and released by exact token generation only after answer checkpoint, rejection, or abort; duplicate completed releases are no-ops, and concurrent prompts survive fresh file/DBOS instances without lost updates, legacy-slot underflow, or CWD-dependent checkpoint identities. Answering while quit/paused cannot advance workflow code until explicit resume, and every later quit installs a fresh barrier. Multi-stage resume settles every acknowledgement, re-reads actual stage/control state, retries only still-paused targets, guards terminal runs, flushes the durable running transition, retries a still-paused durable handle on a later request after transient persistence failure, and reports partial/late/durable failures without calling visible work a no-op.
