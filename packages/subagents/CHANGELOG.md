...
- Fixed async event persistence so cumulative `message_update.message` and `assistantMessageEvent.partial` snapshots are not archived per delta. Compact incremental deltas plus raw child stdout/stderr share one buffered byte budget and truncation marker, while finalized messages and later lifecycle/control/terminal records remain available; asynchronous stream-open failures are non-fatal and concurrent closes await the same drain.
...
- Fixed stale-run repair to stage the exact public result beside its destination, commit terminal status, and atomically consume the hidden stage when publishing; interrupted renames resume without leaving a replayable stage beside a delivered result. The result watcher defers modern `asyncDir` results until status is terminal, canonicalizes aliases by `runId`, revalidates session ownership after delivery waits, retries with exponential backoff, and commits only after acknowledged idempotent Intercom and local notifications while preserving legacy result-only compatibility and exactly-once side effects.
