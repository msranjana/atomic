...
Tests will be written first. The core red test should demonstrate that, from the repository root, `resolveSkills(["tdd", "playwright-cli"], process.cwd())` resolves to `packages/subagents/skills/*/SKILL.md` instead of returning both names in `missing`.
...
Evidence from investigation: a Bun script using `DefaultResourceLoader` with `builtinPackagePaths: [resolve("packages/subagents")]` resolved `playwright-cli`, `tdd`, and `subagent` to `packages/subagents/skills/*/SKILL.md`.
...
- `packages/subagents/skills/playwright-cli/SKILL.md`
...
1. From repo root, resolving `tdd` and `playwright-cli` through `packages/subagents/src/agents/skills.ts` succeeds and resolves to `packages/subagents/skills`.
2. Foreground subagent runs using builtin `debugger` no longer warn `Skills not found: tdd, playwright-cli` solely because the skills are builtin package resources.
...
3. Do not change the `debugger` agent’s declared `skills: tdd, playwright-cli`.
...
flowchart TD
  ...
  MainSkills[Main chat skills list<br/>tdd/playwright-cli visible]

  SubagentTool[@bastani/subagents extension<br/>packages/subagents/src/extension/index.ts]
  AgentDef[debugger.md<br/>skills: tdd, playwright-cli]
...
| `packages/subagents/agents/debugger.md` | Declares `skills: tdd, playwright-cli` | Markdown + YAML frontmatter | Reproduction target for issue #1087 |
...
| `test/unit/coding-agent-builtin-workflows.test.ts` (optional update) | Existing builtin package ResourceLoader coverage | `bun:test` | Can assert main loader also exposes `tdd` and `playwright-cli` |
...
1. Caller requests skills from run options or agent config:
   - `debugger.md` declares `tdd, playwright-cli`.
...
5. Filesystem scan discovers `packages/subagents/skills/tdd/SKILL.md` and `packages/subagents/skills/playwright-cli/SKILL.md`.
...
- `discoverAvailableSkills(cwd)` feeds selection/doctor surfaces and should include `tdd` and `playwright-cli` after the fix, while still filtering `subagent`.
...
3. Optionally update `test/unit/coding-agent-builtin-workflows.test.ts` to assert ResourceLoader exposes `tdd` and `playwright-cli`.
...
1. **Builtin resolver regression from repo root**
   ...
   - Act: `resolveSkills(["tdd", "playwright-cli"], repoRoot)`.
   - Assert:
     - `missing` is empty.
     - resolved names include `tdd` and `playwright-cli`.
...
2. **Injection content regression**
   ...
   - Assert:
     ...
     - contains `<skill name="playwright-cli">`;
...
5. **Optional ResourceLoader parity assertion**
   ...
   - Add `tdd` and `playwright-cli` to expected builtin skill names, alongside current checks for `subagent` and `intercom`.
